...
describe ("getCandidatesForLRP", function(){
    ...
    before(function(){
        collection = {find: function(){return;}} as Collection;
        ...
        const findStub = sinon.stub(collection, "find").onCall(0).returns({toArray: function(){return Promise.resolve([nodeOne]);}} as FindCursor<unknown>);
        findStub.onCall(1).returns({toArray: function(){return Promise.resolve([nodeTwo, nodeThree]);}} as FindCursor<unknown>);
    });
    ...
    it("Returns candidate nodes for a query", async function(){
        const result = await getCandidatesForLRP(LRP as LRPObject, collection);
